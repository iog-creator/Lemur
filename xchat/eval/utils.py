# This code is based on https://github.com/allenai/open-instruct.
...
from xchat.eval.dispatch_openai_requests import dispatch_openai_chat_requesets, dispatch_openai_prompt_requesets
...
class KeyWordsCriteria(StoppingCriteria):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor, **kwargs) -> bool:
        ...
        for i in range(input_ids.shape[0]):
            sequence_should_be_stopped = any(
...
def query_openai_chat_model(
    ...
    organized_message_format=False,
    assigned_temperatures=None,
    **completion_kwargs,
):
    ...
    # by default, we use temperature 0.0 to get the most likely completion.
    generate_temperature = completion_kwargs.pop("temperature", 0.0)

    results = []
    if output_path is not None:
        fout = open(output_path, "w")  # noqa: SIM115
